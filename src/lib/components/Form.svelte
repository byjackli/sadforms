<script>
  ...
  async function updateFeedback(fieldid, groupid, validation) {
    ...
    if (!block?.classList.contains("active")) block.classList.toggle("active");
  ...
  async function onFocus(fieldid, groupid) {
    ...
    if (getEntry(uid, "redact", fieldid, groupid)) fieldValue(fieldid, groupid);
    if (getEntry(uid, "validity", fieldid, groupid)) {
  ...
  async function loadField(field, group) {
    ...
    if (!fieldData(uid, { action: "exists" }, field.uid, g?.uid)) {
      ...
      fieldData(uid, { verdict, action: "init", data }, field.uid, g?.uid);
...
<div id={uid} class="sf sf-container">
    ...
    >
      {#each fieldsArr as group (group)}
        {#if group.meta}
          <div
            class={`form-group${
              group.meta.class ? ` ${group.meta.class}` : ""
            }`}
            role="group"
  ...
  {:else}
    <div>
      ...
      <p>
        If you believe it should be, please contact the developer and the person
